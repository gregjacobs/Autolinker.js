 ...
 *
 * Matcher to find URL matches in an input string. See this class's superclass 
 * ({@link Autolinker.matcher.Matcher}) for more details on Matchers.
 * 
...
export class UrlMatcher extends Matcher {
	...
	parseMatches( text: string ) {
		...
		const table = new CliTable( {
			head: [ 'charIdx', 'char', 'state', 'charIdx', 'currentUrl.idx', 'hasDomainDot' ]
		} );
		...
		while( charIdx < len ) {
			...
			// For debugging: search for other "For debugging" lines
			table.push( 
				[ charIdx, char, State[ state ], charIdx, currentUrl.idx, currentUrl.hasDomainDot ] 
			);
			...
			switch( state ) {
				...
				// States for when we're not sure yet if we're reading a scheme or a domain yet
				case State.SchemeOrDomainChar: stateSchemeOrDomainChar( char ); break;
				case State.SchemeOrDomainDotChar: stateSchemeOrDomainDotChar( char ); break;
				case State.SchemeOrDomainHyphenChar: stateSchemeOrDomainHyphenChar( char ); break;
				case State.SchemeOrPortColon: stateSchemeOrPortColon( char ); break;
		...
		function stateNonUrl( char: string ) {
			if( char === '/' ) {
				...
				beginUrl( State.ProtocolRelativeSlash1, { type: 'scheme', isProtocolMatch: true, isProtocolRelative: true } );

			} else if( letterRe.test( char ) ) {
				// An uppercase or lowercase letter may start either a scheme
				// or a domain name label
				beginUrl( State.SchemeOrDomainChar, { type: 'unknown' } );

			} else if( alphaNumericCharsRe.test( char ) ) {
				// A digit or unicode alpha character could start a domain name label
				beginUrl( State.DomainLabelChar, { type: 'tld' } );
		...
		function stateSchemeColon( char: string ) {
			// TODO: Set isProtocolMatch: true on the currentUrl (somewhere - here?)
		}


		// Handles when we have read a domain label character (and we know that
		// we are in the domain state rather than the SchemeOrDomainChar state)
		...
		function stateDomainDot( char: string ) {
			if( char === '.' ) {
				...
				currentUrl = new CurrentUrl( { ...currentUrl, hasDomainDot: true } );
			...
			if( char === '.' || char === '+' || char === '-' ) {
				...
				if( matchWasCaptured ) {
					...
					currentUrl = new CurrentUrl( { 
						...currentUrl,
						type: 'scheme',
						hasDomainDot: false  // now that we've switched from 'unknown' to 'scheme', any dot we found earlier does not apply to the domain
					} );
				}
				...
				currentUrl = new CurrentUrl( { 
					...currentUrl,
					hasDomainDot: true  // we found a dot and then a valid scheme or domain character
				} );
		...
		function beginUrl(
			newState: State.ProtocolRelativeSlash1 | State.SchemeOrDomainChar | State.DomainLabelChar,
		...
		*/
		...
		function captureMatchIfValid(): boolean {
			if( currentUrl.isValid() ) {  // we need at least one dot in the domain to be considered a valid email address
				...
				// URL begins with 'www.'
				let urlMatchType = currentUrl.type as UrlMatchTypeOptions;
				if( /^www\./.test( url ) ) {
					urlMatchType = 'www';
				}
				...
				matches.push( new UrlMatch( {
					...
					url                   : url,
					protocolUrlMatch      : currentUrl.isProtocolMatch,
...
enum State {
	...
	// Scheme-specific states
	// -SchemeStartChar,      // Schemes must begin with a letter, but this state can never be encountered since domain labels may also begin with a letter. Hence, State.SchemeOrDomainStartChar would be used instead
	SchemeChar,               // Subsequent scheme characters can be: ALPHA / DIGIT / "+" / "-" / "."
	SchemeColon,              // Once we've reached the colon character after a scheme name
	SchemeSlash1,             
	SchemeSlash2,             // Note: If we read two '/'s after the SchemeOrPortColon, then we know we're reading a scheme rather than a domain
	...
	// States for when we're not sure yet if we're reading a scheme or a domain yet
	SchemeOrDomainChar,       // Schemes and domains can begin with an ASCII letter (if it's a digit or unicode letter, it must be a domain label). After the first char, ASCII letters and digits are allowed. If a '+' is encountered, it must be a scheme rather than a domain label
	SchemeOrDomainDotChar,    // Note: If we encounter two dots in a row, it can be part of a scheme, but can no longer be a domain
	SchemeOrDomainHyphenChar, // Schemes may end in a hyphen, but domains may not
	SchemeOrPortColon,        // If we were reading what could be either a scheme or domain, the colon could mean its either the end of the scheme or the start of the port number
...
class CurrentUrl {
	...
	readonly type: CurrentUrlType;
	readonly isProtocolMatch: boolean;
	...
	constructor( cfg: Partial<CurrentUrl> = {} ) {
		...
		this.isProtocolMatch = !!cfg.isProtocolMatch;
		this.isProtocolRelative = !!cfg.isProtocolRelative;
		this.hasDomainDot = !!cfg.hasDomainDot;
	}

	isValid() { 
		// TODO: Scheme matches should probably be valid with any format, but 
		// for a TLD match, it's only valid if we have a valid domain host
		return this.hasDomainDot;
	}
}

type CurrentUrlType = 'unknown' | 'scheme' | 'tld';  // 'unknown' is for when we don't know if it's a scheme or tld match yet